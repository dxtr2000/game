...
vec3 ambient = materialAmbientColor * lightAmbientColor; 
    ...
    vec4 reflectColor = vec4(texture(skybox, reflectDir).rgb, 0.5);

    out_color = texture(textureSampler, v_texture) * 0.1 + reflectColor * 0.5 + vec4(materialEmissionColor + ambient + diffuse + specular, 1.0);
